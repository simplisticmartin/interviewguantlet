...
## 3. MCP architecture (roadmap phase 4)

Not yet implemented. The design, so the seams already exist:
...
`gauntlet/execution/static_check.py` is the seam. It already produces
`interviewer_signals` from structure alone — nested loops, no empty-input guard,
recursion — and returns `executed: false` everywhere. Phase 4 replaces its internals and
adds real test results; the interviewer's consumption of signals does not change.

---

## 4. Ingestion pipeline (roadmap phase 10)

Not implemented. The schema is, which is the part that is expensive to retrofit:
...
and nothing else. The boundaries that are genuinely separate — code execution, MCP tool
